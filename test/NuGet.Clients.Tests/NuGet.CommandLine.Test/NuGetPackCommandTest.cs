      ...
      <dependency id='p2' version='1.5.11' />
        ...
        public void PackCommand_NuspecFileWithTokens_falls_back_to_assembly_version_when_invalid_informational_version() // @odalet, https://github.com/NuGet/Home/issues/5548
        {
            ...
            using (var workingDirectory = TestDirectory.Create())
            {
                ...
                var outputPackage = Path.Combine(projectDirectory, $"Foo.{version}.nupkg"); // The assembly version was used, not the informational version

                // Assert
                Assert.True(0 == r.Item1, r.Item2 + " " + r.Item3); // Exited successfully
                Assert.True(File.Exists(outputPackage), outputPackage);
            }
        }
        ...
        public void PackCommand_NuspecFileWithTokens_uses_informational_version() // @odalet, https://github.com/NuGet/Home/issues/5548
        {
            ...
            using (var workingDirectory = TestDirectory.Create())
            {
                ...
                // Act
                var r = CommandRunner.Run(nugetexe, projectDirectory, "pack Foo.csproj -build", waitForExit: true);
                var outputPackage = Path.Combine(projectDirectory, $"Foo.{semverVersion}.nupkg"); // The informational version without the build metadata part was used
...
[assembly: AssemblyInformationalVersion(""[[informational-version]]"")]
    ...
    public class Class1
    {
        public int A { get; set; }
            ...
            Util.CreateFile(projectDirectory, "file1.cs", csharpContent
                ...
                .Replace("[[file-version]]", fileVersion)
                .Replace("[[informational-version]]", informationalVersion));
